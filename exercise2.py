...
def implement_execute_tool_call(tool_call: Dict, available_tools: List[Tool]) -> str:
    ...
    for tool in available_tools:
        if tool.name == tool_name:
            # Extract the text argument and pass it directly
            text_input = tool_args.get('text', '')
            result = tool.invoke(text_input)
            return str(result)
...
def implement_run_agent_with_tool_calling(model_with_tools: ChatOpenAI, 
    ...
    if implement_check_for_tool_calls(response):
        tool_call = response.tool_calls[0]  # Assuming single tool call for simplicity
        tool_result = implement_execute_tool_call(tool_call, available_tools)
        return tool_result
    else:
